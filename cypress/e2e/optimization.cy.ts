describe('Optimization Domain', () => {
  ...
  it('should search for optimizations using existing test data', () => {
    cy.visit('/optimizations')
    ...
    // Check if table has data, if so get first optimization name to search for
    cy.get('body').then(($body) => {
      const hasRows = $body.find('table tbody tr').length > 0
      
      if (hasRows) {
        // Get the first optimization name from the table to search for
        cy.get('table tbody tr').first().find('td').first().invoke('text').then((firstItemName) => {
          ...
          if (trimmedName.length > 0) {
            ...
            // Search for optimizations using partial name
            cy.get('[data-automation-id="optimization-list-search"]').find('input').type(searchTerm)
            // Wait for debounce (300ms) plus API call
            cy.wait(800)
            
            // Verify the search results contain the optimization (should find at least the one we searched for)
        ...
        cy.get('[data-automation-id="optimization-list-search"]').find('input').should('exist')
        cy.get('[data-automation-id="optimization-list-search"]').find('input').type('test')
        cy.wait(800)
        cy.get('[data-automation-id="optimization-list-search"]').find('input').clear()
      }
    })
  ...
  it('should search for optimizations and filter results', () => {
    cy.visit('/optimizations')
    ...
    cy.get('table tbody tr').then(($rows) => {
      ...
      cy.get('[data-automation-id="optimization-list-search"]').find('input').type('nonexistent-search-term-xyz')
      // Wait for debounce (300ms) plus API call
      cy.wait(800)
  ...
  it('should not have a new optimization button (read-only)', () => {
    cy.visit('/optimizations')
    cy.get('button').contains('New Optimization').should('not.exist')
  })
})
