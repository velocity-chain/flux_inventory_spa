<template>
  <v-container>
    <v-row>
      <v-col>
        <h1 class="text-h4 mb-4">Warehouses</h1>
      </v-col>
    </v-row>
    ...
    <v-row>
      <v-col cols="12">
        <ListPageSearch
          ...
          automation-id="warehouse-list-search"
        />
      </v-col>
    </v-row>
    ...
    <v-row>
      <v-col>
        <v-card>
          <v-data-table
            :headers="headers"
            :items="(warehouses ?? []) as unknown as Warehouse[]"
            :loading="isLoading as unknown as boolean"
            @click:row="navigateToWarehouse"
            hover
            :items-per-page="-1"
            hide-default-footer
          >
          ...
          <v-card-actions v-if="hasMoreValue">
            <v-btn
              ...
              color="primary"
              block
              data-automation-id="warehouse-list-load-more"
            >
    ...
    <v-snackbar :model-value="showError as unknown as boolean" color="error" :timeout="5000">
      Failed to load warehouses: {{ errorMessage }}
    </v-snackbar>
  </v-container>
</template>
...
/**
 * Warehouses List Page - Showcase of flux_spa_utils simplicity
 * 
...
const {
  ...
  setSortBy,
  setOrder,
} = useInfiniteScroll<Warehouse>({
  queryKey: ['warehouses'],
  queryFn: (params) => api.getWarehouses(params),
  getItemId: (item) => item._id,
  limit: 20,
})

function navigateToWarehouse(_event: unknown, { item }: { item: Warehouse }) {
