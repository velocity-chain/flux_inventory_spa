<template>
  <v-container>
    <v-row>
      <v-col>
        <h1 class="text-h4 mb-4">Edit Product</h1>
      </v-col>
    </v-row>
        ...
        <v-card>
          <v-card-text>
            <AutoSaveField
              ...
              automation-id="product-edit-name-input"
            />
            ...
            <AutoSaveField
              ...
              textarea
              :rows="3"
              automation-id="product-edit-description-input"
            />
            ...
            <AutoSaveSelect
              ...
              class="mt-4"
              automation-id="product-edit-status-select"
            />
            ...
            <v-row>
              <v-col cols="12" md="6">
                <v-text-field
                  :model-value="formatDate(product.created.at_time)"
                  label="Created"
                  readonly
                  variant="outlined"
                  density="compact"
                />
                <v-text-field
                  :model-value="product.created.by_user"
                  label="Created By"
                  readonly
                  variant="outlined"
                  density="compact"
              ...
              <v-col cols="12" md="6">
                <v-text-field
                  :model-value="formatDate(product.saved.at_time)"
                  label="Last Saved"
                  readonly
                  variant="outlined"
                  density="compact"
                />
                <v-text-field
                  :model-value="product.saved.by_user"
                  label="Last Saved By"
                  readonly
                  variant="outlined"
                  density="compact"
            ...
            <v-card-actions class="px-0 mt-4">
              <v-btn 
                ...
                variant="text"
                data-automation-id="product-edit-back-button"
              >
                Back to List
              </v-btn>
            </v-card-actions>
...
/**
 * Product Edit Page - Showcase of spa_utils AutoSave components
 * 
...
const { mutateAsync: updateProduct } = useMutation({
  mutationFn: (data: ProductUpdate) => api.updateProduct(productId.value, data),
  onSuccess: () => {
...
async function updateField(field: keyof ProductUpdate, value: string) {
  try {
    await updateProduct({ [field]: value })
  } catch (error) {
    throw error
  }
}
